...
function CityTaxBalance(cix: integer; const CityReport: TCityReportNew)
  : integer;
...
function ProcessEnhancement(uix: integer; const Jobs: TEnhancementJobs)
  : integer;
...
begin
  ...
  result := (Loc + (dx + y0 and 1 + G.lx * 1024) shr 1) mod G.lx + G.lx
    * (y0 + dy)
end;
...
begin
  inc(Loc0, G.lx * 1024);
  inc(Loc1, G.lx * 1024);
  dx := abs(((Loc1 mod G.lx * 2 + Loc1 div G.lx and 1) - (Loc0 mod G.lx * 2 +
    Loc0 div G.lx and 1) + 3 * G.lx) mod (2 * G.lx) - G.lx);
...
begin
  ...
  begin
    result := FoodSurplus;
    if not IsCityAlive or (result > 0) and
      ((gov = gFuture) or (size >= NeedAqueductSize) and (result < 2)) then
      result := 0; { no growth }
  end;

  function CityTaxBalance(cix: integer;
    const CityReport: TCityReportNew): integer;
  var
    i: integer;
  begin
    result := 0;
    ...
    begin
      ...
      if ((MyRO.Government = gFuture) or (MyCity[cix].size >= NeedAqueductSize)
        and (CityReport.FoodSurplus < 2)) and (CityReport.FoodSurplus > 0) then
  ...
  procedure GetUnitInfo(Loc: integer; var uix: integer;
    var UnitInfo: TUnitInfo);
  var
    i, Cnt: integer;
  begin
  ...
  procedure GetCityInfo(Loc: integer; var cix: integer;
    var CityInfo: TCityInfo);
  begin
    if MyMap[Loc] and fOwned <> 0 then
    begin
      CityInfo.Loc := Loc;
        ...
        for dx := -2 to 2 do
          for dy := -2 to 2 do
            if abs(dx) + abs(dy) = 2 then
              if Server(sMoveUnit - sExecute + dx and 7 shl 4 + dy and 7 shl 7,
                me, uix, nil^) >= rExecuted then
                result := false;
  end;
        ...
        case Domain of
          ...
          dSea:
            begin
              ...
              assert(Defense < 6097);
              assert(Cost < 4381);
              Hash1 := ((Attack * 6097 + Defense) * 5 + (Speed - 350)
                div 100) * 2;
              if Weight >= 6 then
                inc(Hash1);
  ...
  function ProcessEnhancement(uix: integer;
    const Jobs: TEnhancementJobs): integer;
  { return values:
  ...
  begin
    ...
    begin
      ...
      procedure CityOptimizer_CityChange(cix: integer);
      begin
        if (MyRO.Government <> gAnarchy) and
          (MyCity[cix].Flags and chCaptured = 0) then
        begin
          CityNeedsOptimize[cix] := true;
