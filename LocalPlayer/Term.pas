...
begin
  ...
  procedure TMainScreen.Client(Command, NewPlayer: integer; var Data);
    ...
    begin
      ...
      if ok then
        repeat
          SearchRec.Name := Copy(SearchRec.Name, 1,
            Length(SearchRec.Name) - 10);
    ...
    begin
      ...
      begin
        ...
        for i := 0 to nPl - 1 do
          ...
          begin
            TestColorDistance :=
              abs(integer(UnusedTribeFiles.Objects[j]) shr 16 and
              $FF - Tribe[i].Color shr 16 and $FF) +
    ...
    begin
      ...
      for x := 0 to 11 do
        for y := 0 to 1 do
          MiniColors[x, y] := GrExt[HGrSystem].Data.Canvas.Pixels
            [66 + x, 67 + y];
  ...
  const
    ...
    const
      nStartBlock = 27;
      StartBlock: array [0 .. nStartBlock - 1] of string = ('INVALID',
        'TURNEND', 'DISBAND', 'CHEAT', 'MSG_DEFAULT', 'WARNING_DISORDER',
        'WARNING_FAMINE', 'WARNING_LOWSUPPORT', 'WARNING_LOWFUNDS',
        'MOVE_MOUNTAIN', 'MOVE_LOAD', 'MOVE_UNLOAD', 'MOVE_DIE', 'NOMOVE_TIME',
        'NOMOVE_DOMAIN', 'NOMOVE_DEFAULT', 'CITY_SELLIMP', 'CITY_REBUILDIMP',
        'CITY_BUYPROJECT', 'CITY_UTILIZE', 'NEWMODEL_0', 'NEWADVANCE_0',
        'AGE_0', 'REVOLUTION', 'NEWGOV', 'CITY_INVALIDTYPE', 'MSG_GAMEOVER');
      ...
      ScienceBlock: array [0 .. nScienceBlock - 1] of string =
        ('MOVE_PARACHUTE', 'MOVE_PLANESTART', 'MOVE_PLANELANDING',
        'MOVE_COVERT', 'NEWMODEL_1', 'NEWMODEL_2', 'NEWMODEL_3', 'NEWADVANCE_1',
        'NEWADVANCE_2', 'NEWADVANCE_3', 'AGE_1', 'AGE_2', 'AGE_3', 'SHIP_BUILT',
        'SHIP_TRADED', 'SHIP_CAPTURED', 'SHIP_DESTROYED');
    ...
    const
      ...
      AdvBookIcon: array [0 .. nAdvBookIcon - 1] of record Adv,
        ...
        Icon: woLighthouse), (Adv: adPoetry; Icon: imTheater),
        (Adv: adMonotheism; Icon: woMich), (Adv: adPhilosophy; Icon: woLeo),
        (Adv: adTheoryOfGravity; Icon: woNewton), (Adv: adSteel;
        Icon: woEiffel), (Adv: adDemocracy; Icon: woLiberty),
        (Adv: adAutomobile; Icon: imHighways), (Adv: adSanitation;
        Icon: imSewer), (Adv: adElectronics; Icon: woHoover),
    ...
    begin
      ...
      if (ToldSlavery < 0) or
        ((ToldSlavery = 1) <> (MyRO.Wonder[woPyramids].EffectiveOwner >= 0))
      then
      begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            TribeMessage(me,
              Format(s, [Phrases.Lookup('CREDIBILITY',
        ...
        begin
          ...
          for cix := 0 to MyRO.nCity - 1 do
            if (MyCity[cix].Loc >= 0) and
              (MyCity[cix].ID = MyRO.Wonder[i].CityID) then
              OwnWonder := true;
      ...
      begin // tell about new techs
        for ad := 0 to nAdv - 1 do
          if (MyRO.TestFlags and tfAllTechs = 0) and
            ((MyRO.Tech[ad] >= tsApplicable) <>
            (MyData.ToldTech[ad] >= tsApplicable)) or (ad in FutureTech) and
          ...
          for cix := 0 to MyRO.nCity - 1 do
            with MyCity[cix] do
              if (MyRO.Turn > 0) and (Loc >= 0) and (Flags and chCaptured = 0)
                and (WondersOnly = (Flags and chProduction <> 0) and
                  ...
                  begin // !!! Shinkansen
                    ...
                    begin
                      ...
                      begin
                        ...
                        if (MoveResult < rExecuted) or
                          (MoveResult = eEnemySpotted) then
                        begin
                          ok := false;
                          Break
                        end;
  ...
  begin { >>>client }
    case Command of
      cTurn, cResume, cContinue, cMovieTurn, scContact,
    ...
    case Command of
          ...
          begin
            ...
            begin
              ...
              case Offer.Price[i] and opMask of
                ...
                opTech:
                  s := s + Phrases.Lookup('ADVANCES',
                    Offer.Price[i] and $FFFFFF);
                opAllTech:
                  s := s + 'All advances';
                opModel:
      ...
      cGetReady, cReplay:
        ...
        begin
          ...
          begin
            for p1 := 0 to nPl - 1 do
              if (G.Difficulty[p1] > 0) and (Tribe[p1] = nil) and
                (G.RO[p1] <> nil) then
              ...
              begin
                ...
                begin // allied unit -- make discovered land visible
                  ...
                  with MyRO.EnemyModel[emix] do
                    if (Kind = mkDiplomat) or (Domain = dAir) or
                      (ATrans_Fuel > 0) or
                      (Cap and (1 shl (mcRadar - mcFirstNonCap) or
        ...
        begin
          ...
          for j := 0 to nSaveOption - 1 do
            if Components[i].Tag and $FF = SaveOption[j] then
              TMenuItem(Components[i]).Checked := 1 shl j and
                OptionChecked <> 0;
        end
  ...
  begin
    ...
    SetWindowPos(sb.h, 0, xRightPanel + 10 - 14 -
      GetSystemMetrics(SM_CXVSCROLL), ClientHeight - MidPanelHeight + 8, 0, 0,
          ...
          begin
            ...
            begin
              ...
              Server(sGetHypoCityTileInfo, me, dLoc(MyCity[cix].Loc, dx, dy),
                TileInfo);
              if (TileInfo.Food >= NeedFood) and (TileInfo.Prod >= NeedProd)
              then
              begin
                result := true;
                exit
  ...
  begin
    ...
    if (width <= 0) or (height <= 0) or (width + xMap <= 0) or
      (height + yMap <= 0) then
      exit;

    NoMap.BitBlt(Panel, -xMap - MapOffset, -yMap + MapHeight - overlap,
      xMidPanel, overlap, 0, 0, SRCCOPY);
    NoMap.BitBlt(Panel, -xMap - MapOffset + xRightPanel,
      -yMap + MapHeight - overlap, Panel.width - xRightPanel, overlap,
      xRightPanel, 0, SRCCOPY);
    if yMap < 0 then
    begin
      if xMap < 0 then
        BitBlt(Canvas.Handle, MapOffset, TopBarHeight, width + xMap,
          height + yMap, Buffer.Canvas.Handle, -xMap, -yMap, SRCCOPY)
      else
        BitBlt(Canvas.Handle, xMap + MapOffset, TopBarHeight, width,
          height + yMap, Buffer.Canvas.Handle, 0, -yMap, SRCCOPY)
    end
    else
    begin
      if xMap < 0 then
        BitBlt(Canvas.Handle, MapOffset, TopBarHeight + yMap, width + xMap,
          height, Buffer.Canvas.Handle, -xMap, 0, SRCCOPY)
      else
        BitBlt(Canvas.Handle, xMap + MapOffset, TopBarHeight + yMap, width,
          height, Buffer.Canvas.Handle, 0, 0, SRCCOPY);
    end
  end;
    ...
    begin
      ...
      for x := 0 to G.lx - 1 do
        ...
        begin
          ...
          for i := 0 to 1 do
          begin
            xm := ((x - xwMini) * 2 + i + y and 1 - hw + G.lx * 5)
              mod (G.lx * 2);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if xw < xwd then
            ProcessRect(xw, ywd, (xwd - xw) * 2 - 1, MapHeight div yyt - ywd +
              yw, ProcessOptions)
          else if xw > xwd then
        ...
        begin
          if DoInvalidate then
            ...
            ProcessRect(xw, (ywd + MapHeight div (yyt * 2) * 2),
              MapWidth div xxt, yw - ywd + 1, ProcessOptions);
          if xw < xwd then
  ...
  begin
    BitBlt(Panel.Canvas.Handle, xMini + 2, yMini + 2, G.lx * 2, G.ly,
      Mini.Canvas.Handle, 0, 0, SRCCOPY);
    if MarkCityLoc >= 0 then
      Sprite(Panel, HGrSystem,
        xMini - 2 + (4 * G.lx + 2 * (MarkCityLoc mod G.lx) +
  ...
  begin
    ...
    begin
      ...
      Fill(Panel.Canvas, xRightPanel + 10 - 7, 3, Panel.width - xRightPanel - 10
        + 7, PanelHeight - 3, -(xRightPanel + 10 - 7),
        hMainTexture - PanelHeight);
      ...
      begin
        ...
        begin // display terrain types
          ...
          begin
            ...
            if xSrcBase >= 0 then
              Sprite(Panel, HGrTerrain, xTroop + 2 + x, yTroop + 9 - yyt,
                xxt * 2, yyt * 3, 1 + xSrcBase * (xxt * 2 + 1),
                1 + ySrcBase * (yyt * 3 + 1));
      ...
      begin
        ...
        if UnFocus >= 0 then
          ...
          begin { display info about selected unit }
            ...
            s := IntToStr(Health) + '%';
            LightGradient(Panel.Canvas, xMidPanel + 7 + 12 + 7,
              PanelHeight - 22, (Health + 1) div 2,
        ...
        begin
          ...
          begin
            ...
            for i := 0 to Count - 1 do
              if (i >= TrRow * sb.si.npos) and (i < TrRow * (sb.si.npos + 1))
              then
        ...
        begin // show terrain and improvements
          ...
          begin
            ...
            PaintRelativeProgressBar(Panel.Canvas, 3, xTerrain - 68, 148 + 3,
              63, JobProgressData[JobFocus].Done,
            ...
            s := Format('%s/%s',
              ...
              ScreenTools.MovementToString(JobProgressData[JobFocus]
              .Required)]);
          ...
          if MyMap[TroopLoc] and (fTerrain or fSpecial) = fGrass or fSpecial1
          then
        ...
        begin
          ...
          begin
            Dump(Panel, HGrSystem, Left, Top - self.ClientHeight + PanelHeight,
              25, 25, 169, 243);
            Sprite(Panel, HGrSystem, Left, Top - self.ClientHeight +
              PanelHeight, 25, 25, 1 + 26 * ButtonIndex, 337);
            RFrame(Panel.Canvas, Left - 1, Top - self.ClientHeight +
              (PanelHeight - 1), Left + width, Top + height - self.ClientHeight
              + PanelHeight, MainTexture.clBevelShade,
    ...
    SmartRectInvalidate(0, ClientHeight - PanelHeight, ClientWidth,
      ClientHeight);
    ...
    begin
      ...
      if MyRO.Government <> gAnarchy then
      begin
        ImageOp_BCC(TopBar, Templates, xTreasurySection + 48, 22, 124, 1, 14,
          14, $0000C0, $0080C0);
        if TaxSum >= 0 then
      ...
      begin
        ...
        PaintRelativeProgressBar(TopBar.Canvas, 2, xResearchSection + 48 + 1,
          26, CostFactor, TrueResearch, ScienceSum, i, true, MainTexture);
  ...
  function TMainScreen.ZoomToCity(Loc: integer;
    NextUnitOnClose: boolean = false; ShowEvent: integer = 0): boolean;
  begin
    ...
    begin
      ...
      begin
        ...
        if ssCtrl in Shift then // toggle special resource
          ...
          EditTileData.NewTile := MyMap[MouseLoc] and not fTerrain or
            fSpecial or BrushType
          ...
          if MyMap[MouseLoc] and (fDeadLands or fModern) = BrushType and
            ...
            EditTileData.NewTile := MyMap[MouseLoc] and
              not(fDeadLands or fModern)
          else
            EditTileData.NewTile := MyMap[MouseLoc] and
              not(fDeadLands or fModern) or BrushType
        ...
        if ywmax <= 0 then
          ...
          Frame(Panel.Canvas, xMini + 2 + G.lx - MapWidth div (2 * xxt),
            ...
            MainTexture.clMark);
        RectInvalidate(xMini + 2, TopBarHeight + MapHeight - overlap + yMini +
          2, xMini + 2 + G.lx * 2, TopBarHeight + MapHeight - overlap + yMini
          + 2 + G.ly)
      end
      ...
      begin
        ...
        else if (MyMap[MouseLoc] and fSpiedOut <> 0) and not(ssCtrl in Shift)
        then
        begin
          DoCenter := false;
          SetTroopLoc(MouseLoc);
        ...
        for p1 := 0 to nPl - 1 do
          ...
          begin
            ...
            for emix := MyRO.nEnemyModel - 1 downto 0 do
              ...
              begin
                if Tribe[p1].ModelPicture[MyRO.EnemyModel[emix].mix].HGr = 0
                then
                  InitEnemyModel(emix);
  ...
  var
    ...
    OldToTile: Cardinal;
    CityCaptured, IsAttack, OldUnrest, NewUnrest, NeedEcoUpdate,
      NeedRepaintPanel, ToTransport, ToShip: boolean;
    PlaneReturnData: TPlaneReturnData;
    QueryItem: string;
  begin
    result := eInvalid;
    ...
    begin
      ...
      begin // move
        ...
        while (cix >= 0) and (MyCity[cix].Loc <> ToLoc) do
          dec(cix);
        if (result <> eMissionDone) and (MyMap[ToLoc] and fCity <> 0) and
          (cix < 0) then
          CityCaptured := true;
    ...
    begin
      ...
      begin // city destroyed
        for i := 0 to 27 do { tell about destroyed wonders }
          if (MyRO.Wonder[i].CityID = -2) and
            (MyData.ToldWonders[i].CityID <> -2) then
            with MessgExDlg do
            begin
    ...
    begin
      if (UnFocus >= 0) and ((result = eNoTime_Move) or UnitExhausted(UnFocus)
        or (MyUn[UnFocus].Master >= 0) or
        (MyModel[MyUn[UnFocus].mix].Domain = dAir) and
        ((MyMap[MyUn[UnFocus].Loc] and fCity <> 0) { aircrafts stop in cities }
  ...
  var
    ToLoc, xFromLoc, yFromLoc, xToLoc, yToLoc, xFrom, yFrom, xTo, yTo, xMin,
  ...
  begin
    ...
    begin
      ...
      while abs(((xFromLoc - xw1 + G.lx) * 2 + yFromLoc and 1 + 1) * xxt * 2 +
        dx * xxt - MapWidth) < abs(((xFromLoc - xw1) * 2 + yFromLoc and 1 + 1) *
        xxt * 2 + dx * xxt - MapWidth) do
        dec(xw1, G.lx);

      xTo := (xToLoc - xw1) * (xxt * 2) + yToLoc and 1 * xxt + (xxt - xxu);
      yTo := (yToLoc - yw) * yyt + (yyt - yyu_anchor);
      xFrom := (xFromLoc - xw1) * (xxt * 2) + yFromLoc and 1 * xxt +
        (xxt - xxu);
  ...
  begin
      ...
      repeat
        ...
        begin
          ...
          if MoveResult < rExecuted then
            ...
            StopReason := Dead
          else if (StopReason = None) and (MoveResult and rEnemySpotted <> 0)
          then
            StopReason := EnemySpotted;
          if StopReason <> None then
            Break;
        end;
        if (StopReason = None) and
          ((MoveAdviceData.nStep < 25) or
        ...
        case StopReason of
          ...
          begin // multi-turn goto
            if Loc = maNextCity then
              MyUn[uix].Status := MyUn[uix].Status and
                ($FFFF - usStay - usRecover) or usGoto + $7FFF shl 16
            else
              MyUn[uix].Status := MyUn[uix].Status and
                ($FFFF - usStay - usRecover) or usGoto + Loc shl 16;
    ...
    begin
      ...
      begin
        if (x >= xMini + 2) and (y >= yMini + 2) and (x < xMini + 2 + 2 * G.lx)
          and (y < yMini + 2 + G.ly) then
          if ssShift in Shift then
          begin
            xMouse := (xwMini + (x - (xMini + 2) + MapWidth div (xxt * 2) +
              G.lx) div 2) mod G.lx;
            ...
            begin
              ...
              begin
                if ssShift in Shift then
                  ...
                  (x - xTroop - 1 - i * TrPitch >= 60 - 20) and
                  (y >= yTroop + 35) and
                  ((MyUn[trix[i]].Job > jNone) or (MyUn[trix[i]].Status and
                  (usStay or usRecover or usGoto) <> 0)) then
                begin // wake up
                  MyUn[trix[i]].Status := MyUn[trix[i]].Status and
                    ($FFFF - usStay - usRecover - usGoto - usEnhance) or
                    usWaiting;
      ...
      begin
        ...
        with MyUn[UnFocus] do
          ...
          begin
            DestinationMarkON := false;
            PaintDestination;
            Status := Status and ($FFFF - usStay - usGoto - usEnhance) or
              usRecover;
            if Job > jNone then
              Server(sStartJob + jNone shl 4, me, UnFocus, nil^);
            NextUnit(UnStartLoc, true)
          ...
          begin
            ...
            begin
              ...
              PaintDestination;
              Status := Status and
                ($FFFF - usWaiting - usStay - usRecover - usGoto - usEnhance);
              NextUnit(UnStartLoc, true);
            end
            ...
            begin
              ...
              begin
                if MyModel[mix].Domain = dAir then
                  ...
                  (MyMap[Loc] and fCity = 0) and
                  (MyMap[Loc] and fTerImp <> tiBase) then
                  Play('MOVE_PARACHUTE')
                else
                  Play('MOVE_UNLOAD');
                Status := Status and not usWaiting;
            ...
            begin
              ...
              begin
                ...
                if (MyUn[uix].Master = UnFocus) and
                  (MyUn[uix].Movement = integer(MyModel[MyUn[uix].mix].speed))
                then
                begin
    ...
    begin
          ...
          if MyRO.nCity > 0 then
            for i := 28 to nImp - 1 do
              if (i <> imTrGoods) and (Imp[i].Kind = ikCommon) and
                ...
                ((Imp[i].Preq = preNone) or
                (MyRO.Tech[Imp[i].Preq] >= tsApplicable)) then
              begin
      ...
      begin
        ...
        // no researchtech in case just completed
          or (MyRO.TestFlags and (tfAllTechs or tfUncover or
          tfAllContact) <> 0);
      ...
      begin
        ...
        extended := extended and
          ((mox.Kind = mkSettler) or (mox.Kind = mkSlaves) and
          (MyRO.Wonder[woPyramids].EffectiveOwner >= 0)) and
        ...
        mGoOn.Visible := MyUn[UnFocus].Status and (usGoto or usWaiting)
          = usGoto or usWaiting;
        ...
        mSelectTransport.Visible := Server(sSelectTransport - sExecute, me,
          UnFocus, nil^) >= rExecuted;
      end
      ...
      begin
        ...
        if (Tile and fRiver <> 0) and
          (MyRO.Tech[adBridgeBuilding] >= tsApplicable) then
        begin
        ...
        if extended then
        begin
          mRoad.Visible := JobTest(UnFocus, jRoad,
            [eNoBridgeBuilding, eTreaty]);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          BitBlt(Buffer.Canvas.Handle, 0, 0, G.lx * 2, G.ly, Mini.Canvas.Handle,
            0, 0, SRCCOPY);
          if ywmax <= 0 then
            ...
            Frame(Buffer.Canvas, x - xMini - 2 - MapWidth div (xxt * 2), yw,
              ...
              yw + MapHeight div yyt - 2, MainTexture.clMark,
              MainTexture.clMark);
          BitBlt(Panel.Canvas.Handle, xMini + 2, yMini + 2, G.lx * 2, G.ly,
    ...
    begin
      result := (y >= TopBarHeight + MapHeight) or
        (y >= ClientHeight - PanelHeight) and
        ((x < xMidPanel) or (x >= xRightPanel))
    end;
    ...
    begin
      if idle then
        ...
        begin
          ...
          MapBoxMouseDown(Sender, Button, Shift, x - MapOffset,
            y - TopBarHeight)
    end;
    ...
    begin
      if idle then
        PanelBoxMouseUp(Sender, Button, Shift, x,
          y - (ClientHeight - PanelHeight));
    end;
    ...
    begin
      ...
      BitBlt(Canvas.Handle, MapOffset, TopBarHeight, MapWidth,
        MapHeight - overlap, offscreen.Canvas.Handle, 0, 0, SRCCOPY);
      BitBlt(Canvas.Handle, 0, 0, ClientWidth, TopBarHeight,
        TopBar.Canvas.Handle, 0, 0, SRCCOPY);
      if xMidPanel > MapOffset then
        BitBlt(Canvas.Handle, xMidPanel, TopBarHeight + MapHeight - overlap,
          ClientWidth div 2 - xMidPanel, overlap, offscreen.Canvas.Handle,
          xMidPanel - MapOffset, MapHeight - overlap, SRCCOPY)
      else
        BitBlt(Canvas.Handle, MapOffset, TopBarHeight + MapHeight - overlap,
          ClientWidth div 2 - MapOffset, overlap, offscreen.Canvas.Handle, 0,
          MapHeight - overlap, SRCCOPY);
      if xRightPanel < MapOffset + MapWidth then
        BitBlt(Canvas.Handle, ClientWidth div 2, TopBarHeight + MapHeight -
          overlap, xRightPanel - ClientWidth div 2, overlap,
          offscreen.Canvas.Handle, ClientWidth div 2 - MapOffset,
          MapHeight - overlap, SRCCOPY)
      else
        BitBlt(Canvas.Handle, ClientWidth div 2, TopBarHeight + MapHeight -
          overlap, MapOffset + MapWidth - ClientWidth div 2, overlap,
          offscreen.Canvas.Handle, ClientWidth div 2 - MapOffset,
          MapHeight - overlap, SRCCOPY);
      BitBlt(Canvas.Handle, 0, TopBarHeight + MapHeight - overlap, xMidPanel,
        overlap, Panel.Canvas.Handle, 0, 0, SRCCOPY);
      BitBlt(Canvas.Handle, xRightPanel, TopBarHeight + MapHeight - overlap,
        Panel.width - xRightPanel, overlap, Panel.Canvas.Handle, xRightPanel,
        0, SRCCOPY);
      BitBlt(Canvas.Handle, 0, TopBarHeight + MapHeight, Panel.width,
        PanelHeight - overlap, Panel.Canvas.Handle, 0, overlap, SRCCOPY);
      if (pLogo >= 0) and (G.RO[pLogo] = nil) and (AILogo[pLogo] <> nil) then
        BitBlt(Canvas.Handle, xRightPanel + 10 - (16 + 64),
          ClientHeight - PanelHeight, 64, 64, AILogo[pLogo].Canvas.Handle, 0, 0,
          SRCCOPY);
    end;
    ...
    procedure TMainScreen.SmartRectInvalidate(Left, Top, Rigth,
      Bottom: integer);
    var
      i: integer;
      r0, r1: HRgn;
