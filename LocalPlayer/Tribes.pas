...
type
        ...
        case Kind of
          mkSelfDeveloped, mkEnemyDeveloped:
            case Domain of { age determination }
              dGround:
                if (Attack >= Defense * 4) or (Attack > 0) and (MaxUpgrade < 10)
  ...
  begin
    xp := 0;
    while (xp < xmax) and (GrExt[HGr].Data.Canvas.Pixels[x + 1 + xp, y]
      <> Mark) do
      inc(xp);
    yp := 0;
    while (yp < ymax) and (GrExt[HGr].Data.Canvas.Pixels[x, y + 1 + yp]
      <> Mark) do
      inc(yp);
  end;
...
{$IFNDEF SCR} else
      result := Format(TPhrase('GENCITY'), [i + 1]){$ENDIF}
  end;
...
{$ENDIF}
    ...
    begin
      ...
      begin
        i := 0;
        while (i < Count) and
          (Copy(Strings[i], 1, 6) <> '#AGE' + char(48 + Age) + ' ') do
          inc(i);
        if i < Count then
        begin
          Input := Strings[i];
...
{$ENDIF}
    ...
    procedure TTribe.SetModelPicture(const Info: TModelPictureInfo;
      IsNew: boolean);
    var
      i: integer;
      ok: boolean;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            else if (nPictureList >= 64) and
              (nPictureList and (nPictureList - 1) = 0) then
