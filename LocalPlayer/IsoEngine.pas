  ...
  begin
    ...
    begin
      ...
      repeat
        ...
        procedure TIsoMap.Sprite(HGr, xDst, yDst, Width, Height, xGr,
          yGr: integer);
        begin
          BitBlt(GrExt[HGr].Mask, xDst, yDst, Width, Height, xGr, yGr, SRCAND);
          BitBlt(GrExt[HGr].Data, xDst, yDst, Width, Height, xGr, yGr,
            SRCPAINT);
        end;
...
{$IFNDEF SCR} if Status and usStay <> 0 then
                ...
                j := 16
              else if Status and (usGoto or usEnhance) = usGoto or usEnhance
              then
                j := 18
              ...
              if Flags and unMulti <> 0 then
                Sprite(Tribe[Owner].symHGr, x + xsh - 1 + 4, y + ysh - 2, 14,
                  12, 33 + Tribe[Owner].sympix mod 10 * 65,
              ...
              begin
                ...
                Sprite(HGrStdUnits, x, y, xxu * 2, yyu * 2,
                  1 + 6 * (xxu * 2 + 1), 1);
              end
            end
        end; { PaintUnit }
        ...
        procedure TIsoMap.PaintCity(x, y: integer; const CityInfo: TCityInfo;
          accessory: boolean);
        var
          age, cHGr, cpix, xGr, xShield, yShield, LabelTextColor,
            LabelLength: integer;
          cpic: TCityPicture;
          s: string;
        begin
          ...
          begin
            ...
            if (ciWalled and CityInfo.Flags = 0) or
              (GrExt[cHGr].Data.Canvas.Pixels[(xGr + 4) * 65, cpix * 49 + 48]
              = $00FFFF) then
        ...
        begin
          ...
          if Conn and 1 <> 0 then
            BitBlt(GrExt[HGrTerrain].Mask, x + xxt, y, xxt, yyt,
              1 + 7 * (xxt * 2 + 1) + xxt,
              1 + yyt + 15 * (yyt * 3 + 1), SRCAND);
          if Conn and 2 <> 0 then
            BitBlt(GrExt[HGrTerrain].Mask, x + xxt, y + yyt, xxt, yyt,
              1 + 7 * (xxt * 2 + 1) + xxt, 1 + yyt * 2 + 15 *
              (yyt * 3 + 1), SRCAND);
          if Conn and 4 <> 0 then
            BitBlt(GrExt[HGrTerrain].Mask, x, y + yyt, xxt, yyt,
              1 + 7 * (xxt * 2 + 1), 1 + yyt * 2 + 15 * (yyt * 3 + 1), SRCAND);
          if Conn and 8 <> 0 then
            BitBlt(GrExt[HGrTerrain].Mask, x, y, xxt, yyt,
              1 + 7 * (xxt * 2 + 1), 1 + yyt + 15 * (yyt * 3 + 1), SRCAND);
        end;
        ...
        begin
          ...
          begin
            ...
            if (yLoc = (G.ly - 2) div 4) or (G.ly - 1 - yLoc = (G.ly + 2) div 4)
            then
              Conn := Conn and not 6 // no connection to south
            else if (yLoc = (G.ly + 2) div 4) or
              (G.ly - 1 - yLoc = (G.ly - 2) div 4) then
          ...
          begin
            Conn := Connection4(Loc, fRiver, fRiver) or
              Connection4(Loc, fTerrain, fShore) or
        ...
        procedure TIsoMap.PaintTileObjects(x, y, Loc, CityLoc,
          CityOwner: integer; UseBlink: boolean);
        type
        ...
        var
          ...
          begin
            ...
            if ShowCityNames and (Options and (1 shl moEditMode) = 0) and
              (BehindCityInfo.Loc >= 0) and (BehindCityInfo.Loc < G.lx * G.ly)
            ...
            begin
              ...
              xs := x + xxt - (w + 1) div 2;
              if IsCapital then
                FOutput.Canvas.Font.Style := FOutput.Canvas.Font.Style +
                  [fsUnderline];
          ...
          begin
            if ShowObjects and (Options and (1 shl moEditMode) = 0) and
              (Tile and fCity <> 0) and (CityInfo.Flags and ciSpacePort <> 0)
            then
                ...
                for dy := 0 to 1 do
                  ...
                  begin
                    ...
                    begin
                      ...
                      if p2 <> p1 then
                      begin
                        BitBlt(GrExt[HGrTerrain].Mask, x + dx * xxt,
                          y + dy * yyt, xxt, yyt, 1 + 8 * (xxt * 2 + 1) + dx *
                          xxt, 1 + yyt + 16 * (yyt * 3 + 1) + dy * yyt, SRCAND);
                        BitBlt(Borders, x + dx * xxt, y + dy * yyt, xxt, yyt,
                          dx * xxt, p1 * (yyt * 2) + dy * yyt, SRCPAINT);
                      end
                    end;
                  end
              end
        ...
        begin
          ...
          begin
            ...
            if Tile and fTerrain < fForest then
              ...
              TSprite(x, y, 8 + 2 * 9 + ((Tile and fTerrain - fForest) * 2 +
                Tile and fSpecial shr 5) * 9);
          end;
          ...
          if fog and ShowObjects then
            if Loc < -G.lx then
              Sprite(HGrTerrain, x, y + yyt, xxt * 2, yyt,
                1 + 6 * (xxt * 2 + 1), 1 + yyt * 2 + 15 * (yyt * 3 + 1))
            else if Loc >= G.lx * (G.ly + 1) then
...
{$IFNDEF SCR} if (UnFocus >= 0) and (Loc = MyUn[UnFocus].Loc) then { active unit }
                ...
                begin
                  if ClientMode = cMovieTurn then
                    PaintUnit(x + (xxt - xxu), y + (yyt - yyu_anchor),
                      UnitInfo, 0)
              ...
              Sprite(HGrStdUnits, x + (xxt - xxu), y + (yyt - yyu_anchor),
                xxu * 2, yyu * 2, 1 + 5 * (xxu * 2 + 1), 1)
            else if Tile and fStealthUnit <> 0 then
              Sprite(HGrStdUnits, x + (xxt - xxu), y + (yyt - yyu_anchor),
                xxu * 2, yyu * 2, 1 + 5 * (xxu * 2 + 1), 1 + 1 * (yyu * 2 + 1))
          end;

          if ShowObjects and (Tile and fTerImp = tiFort) and
            (Tile and fObserved <> 0) then
            TSprite(x, y, 3 + 9 * 12);
          ...
          const
            ...
            begin
              ...
              for i := length * 3 mod 4 - 1 downto 0 do
                PByteArray(Line)[rest + i] := PByteArray(Line)
                  [rest + i] shr 1 and $7F;
            end;
        ...
        begin
          ...
          ShowObjects := (CityOwner >= 0) or
            (Options and (1 shl moBareTerrain) = 0);
          ShowCityNames := ShowObjects and (CityOwner < 0) and
            (Options and (1 shl moCityNames) <> 0);
          ShowBorder := true;
          ShowMyBorder := CityOwner < 0;
          ShowGrWall := (CityOwner < 0) and
            (Options and (1 shl moGreatWall) <> 0);
          if ShowDebug then
          ...
          begin
            ...
            RealBottom := y +
              (G.ly - ((Loc + 12345 * G.lx) div G.lx - 12345) + 3) * yyt;
          ...
          for dy := 0 to ny + 1 do
            if (Loc + dy * G.lx >= 0) and (Loc + (dy - 3) * G.lx < G.lx * G.ly)
            then
              for dx := 0 to nx do
              begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      if Aix < 2 then
                        ...
                        bix := Aix;
                    if Aix = -1 then
                      BitBlt(GrExt[HGrTerrain].Data, x + dx * xxt, y + dy * yyt,
                        xxt, yyt, 1 + 6 * (xxt * 2 + 1) + (dx + dy + 1) and
                        1 * xxt, 1 + yyt, SRCCOPY) // arctic <-> ocean
                    else if bix = -1 then
                      BitBlt(GrExt[HGrTerrain].Data, x + dx * xxt, y + dy * yyt,
                        xxt, yyt, 1 + 6 * (xxt * 2 + 1) + xxt - (dx + dy + 1)
                        and 1 * xxt, 1 + yyt * 2, SRCCOPY) // arctic <-> ocean
                    else
              ...
              for dx := -2 to nx + 1 do
                ...
                begin
                  ...
                  if Distance(ALoc, CityLoc) > 5 then
                    PaintTileObjects(x + xxt * dx, y + yyt + yyt * dy, ALoc,
                      CityLoc, CityOwner, UseBlink);
                end;
            dx := ((CityLoc mod G.lx * 2 + CityLoc div G.lx and 1) -
              ((Loc + 666 * G.lx) mod G.lx * 2 + (Loc + 666 * G.lx) div G.lx and
              1) + 3 * G.lx) mod (2 * G.lx) - G.lx;
          ...
          begin
            ...
            for dy := -2 to ny + 1 do
              for dx := -2 to nx + 1 do
                if (dx + dy) and 1 = 0 then
                  PaintTileObjects(x + xxt * dx, y + yyt + yyt * dy,
                    dLoc(Loc, dx, dy), CityLoc, CityOwner, UseBlink);
          end;
