...
type
  ...
  TVector = record
    ...
    class function Create(P1, P2: TPoint): TVector; static;
  end;
...
function PointToLineDistance(const P, V, W: TPoint; out Intersect: TPoint): Integer;
...
function TVector.GetLength: Double;
begin
  Result := Sqrt(Sqr(Direction.X) + Sqr(Direction.Y));
end;

function TVector.GetAngle: Double;
begin
  Result := ArcTan2(Direction.Y, Direction.X);
end;
...
begin
  Angle := GetAngle;
  Direction := Point(Round(Cos(Angle) * Value),
    Round(Sin(Angle) * Value));
end;

class function TVector.Create(P1, P2: TPoint): TVector;
begin
  Result.Position := P1;
  Result.Direction := Point(P2.X - P1.X, P2.Y - P1.Y);
end;
